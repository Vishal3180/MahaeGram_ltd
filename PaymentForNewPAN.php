    ...
    if($billAmount <= $totalbalance){
                  $totalBalance = $totalbalance - $billAmount;  
                    ...
                    if($result)
                    {
                        ...
                        $qry="INSERT INTO `transactionDetails`(`TransactionNumber`, `franchiseID`, `ProductName`, `OperationWallete`, `TransactionAmount`,`TransactionStatus`) VALUES ('$TransactionNumber','$fid','NewPAN','Debit','$billAmount','Success')";
                        $result=mysqli_query($conn,$qry);
                        if($result)
                        {
